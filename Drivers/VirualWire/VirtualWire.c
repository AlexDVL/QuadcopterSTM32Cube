...
//
// Порт на STM32 HAL
// - добавлены define: true и false
// - добавлены функции digitalWrite и digitalRead
// - добавлены string.h и stm32f1xx_hal.h
// - расширение файла изменено с .cpp на .c
// - тип переменных: vw_ptt_pin, vw_ptt_inverted, vw_rx_pin, vw_rx_inverted и
//     vw_tx_pin изменен с uint8_t на uint16_t
// - убарн extern "C"
// - убран выбор платформы
// - убрана (закомментирована) функция vw_wait_rx_max
// - убрано подклчение VirtualWireInterrupts.h
// - убрана функция vw_setup
// - таймер необходимо настраивать самостоятельно:
//   - CTC мод
//   - компаратор = SystemClock/Speed/8
//   - прерывание по сбросу запускает функуию vw_Int_Handler()
//
...
static uint16_t vw_ptt_pin = GPIO_PIN_9;
static uint16_t vw_ptt_inverted = 0;

// The digital IO pin number of the receiver data
static uint16_t vw_rx_pin = GPIO_PIN_7;
static uint16_t vw_rx_inverted = 0;

// The digital IO pin number of the transmitter data
static uint16_t vw_tx_pin = GPIO_PIN_6;
